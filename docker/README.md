...
Following instructions explain how to publish cro tutorial APIs via a portable Docker container.
...
Now if you issue a `docker images` command you should see `cro:latest` image listed.
...
#### From command line
`docker run -d -p <SERVER_PORT>:8080 cro`
